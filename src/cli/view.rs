...
pub(crate) struct RunView {
    ...
    pub(crate) finished_at_utc: Option<String>,
...
impl RunView {
    pub(crate) fn from_run(run: &Run) -> Self {
        ...
        Self {
            ...
            scheduled_for_utc: snapshot.scheduled_for_utc.to_string(),
            started_at_utc: snapshot.started_at_utc.map(|value| value.to_string()),
            finished_at_utc: snapshot.finished_at_utc.map(|value| value.to_string()),
            state: snapshot.state,
